   ...
   public:
    ...
    void addItems(py::object input, py::object ids_ = py::none(), int num_threads = -1, bool replace_deleted = false) {
...

        {
            ...
            // add one point without parallelization
            if (!ep_added) {
    ...
    // MODIFIED: PIVOT SELECTION
    py::object pivot_selection_2L(dist_t radius, int numThreads) {
        printf("2-Layer Pivot Selection: Only Run with Initialized Index\n");

        // run pivot selection
        std::vector<hnswlib::labeltype> pivots{};
        appr_alg->pivot_selection_2L(radius, numThreads, pivots);

        // create the list of pivots
        int numPivots = (int)pivots.size();
        hnswlib::labeltype* data_numpy_l = new hnswlib::labeltype[numPivots];
        for (int p = 0; p < numPivots; p++) {
            data_numpy_l[p] = pivots[p];
        }

        // free the vector
        py::capsule free_when_done_l(data_numpy_l, [](void* f) { delete[] f; });

        return py::make_tuple(py::array_t<hnswlib::labeltype>(
            {numPivots, 1},                                            // shape
            {sizeof(hnswlib::labeltype), sizeof(hnswlib::labeltype)},  // C-style contiguous strides for each index
            data_numpy_l,                                              // the data pointer
            free_when_done_l));
    }

    // MODIFIED: PIVOT SELECTION
    py::object pivot_selection_3L(dist_t radius1, dist_t radius2, int numThreads) {
        printf("Pivot Selection: Only Run with Initialized Index\n");

        std::vector<hnswlib::labeltype> pivots1{};
        std::vector<hnswlib::labeltype> pivots2{};
        appr_alg->pivot_selection_3L(radius1, radius2, numThreads, pivots1, pivots2);

        // create the list of top layer pivots
        int numPivots1 = (int)pivots1.size();
        hnswlib::labeltype* data_numpy_pivots1 = new hnswlib::labeltype[numPivots1];
        for (int p = 0; p < numPivots1; p++) {
            data_numpy_pivots1[p] = pivots1[p];
        }

        // create the list of pivots
        int numPivots2 = (int)pivots2.size();
        hnswlib::labeltype* data_numpy_pivots2 = new hnswlib::labeltype[numPivots2];
        for (int p = 0; p < numPivots2; p++) {
            data_numpy_pivots2[p] = pivots2[p];
        }

        // free the vectors after exit
        py::capsule free_when_done_pivots1(data_numpy_pivots1, [](void* f) { delete[] f; });

        py::capsule free_when_done_pivots2(data_numpy_pivots2, [](void* f) { delete[] f; });

        return py::make_tuple(
            py::array_t<hnswlib::labeltype>({numPivots1},                  // shape
                                            {sizeof(hnswlib::labeltype)},  // C-style contiguous strides for each index
                                            data_numpy_pivots1,            // the data pointer
                                            free_when_done_pivots1),
            py::array_t<hnswlib::labeltype>({numPivots2},                  // shape
                                            {sizeof(hnswlib::labeltype)},  // C-style contiguous strides for each index
                                            data_numpy_pivots2,            // the data pointer
                                            free_when_done_pivots2));
    }

    // MODIFIED: PIVOT SELECTION
    py::object HSP_Graph(py::object input, int k, int num_threads) {
        printf("HSP Graph: Only Run with Initialized Index\n");

        // get the list of pivots from python
        py::array_t<hnswlib::labeltype, py::array::c_style | py::array::forcecast> pivots(input);
        py::buffer_info buffer = pivots.request();
        if (buffer.ndim != 1) {
            char msg[256];
            snprintf(msg, sizeof(msg), "expected input of 1D array\n");
            throw std::runtime_error(msg);
        }
        size_t numPivots = buffer.shape[0];
        hnswlib::labeltype* pivots_pointer = static_cast<hnswlib::labeltype*>(buffer.ptr);

        // turn into vector
        std::vector<hnswlib::labeltype> pivotsVector{};
        pivotsVector.resize(numPivots);
        for (int it1 = 0; it1 < (int)numPivots; it1++) {
            pivotsVector[it1] = pivots_pointer[it1];
        }

        // compute the hsp graph
        std::vector<std::vector<hnswlib::labeltype>> HSP_Graph{};
        appr_alg->computeHSPGraph(pivotsVector, k, num_threads, HSP_Graph);

        // create linked list for output
        size_t num_elements = 0;
        // printf("HSP Graph: \n");
        for (int it1 = 0; it1 < (int)numPivots; it1++) {
            num_elements += 1;                      // storing index of pivot
            num_elements += 1;                      // storing number of neighbors
            num_elements += HSP_Graph[it1].size();  // storing each neighbor
            // printf("  * %u: %d --> {", (unsigned int) pivotsVector[it1], (int) HSP_Graph[it1].size());
            // for (int it2 = 0; it2 < (int) HSP_Graph[it1].size(); it2++) {
            //     printf("%u,", (unsigned int) HSP_Graph[it1][it2]);
            // }
            // printf("}\n");
        }

        // initialize the linked list
        hnswlib::labeltype* data_numpy_linked_list = new hnswlib::labeltype[num_elements];
        size_t count = 0;
        for (int it1 = 0; it1 < (int)numPivots; it1++) {
            data_numpy_linked_list[count++] = (hnswlib::labeltype)pivotsVector[it1];
            data_numpy_linked_list[count++] = (hnswlib::labeltype)HSP_Graph[it1].size();
            for (int it2 = 0; it2 < (int)HSP_Graph[it1].size(); it2++) {
                data_numpy_linked_list[count++] = (hnswlib::labeltype)HSP_Graph[it1][it2];
            }
        }

        // free the vectors after exit
        py::capsule free_when_done_linked_list(data_numpy_linked_list, [](void* f) { delete[] f; });

        return py::array_t<hnswlib::labeltype>(
            {num_elements},                // shape
            {sizeof(hnswlib::labeltype)},  // C-style contiguous strides for each index
            data_numpy_linked_list,        // the data pointer
            free_when_done_linked_list);
    }

    // sets of
    void set_pivots_and_links(py::object input_layer_1, py::object input_layer_2, int M) {
        printf("Setting Links on HNSW\n");

        // not today junior

        // load list for layer 1
        py::array_t<hnswlib::labeltype, py::array::c_style | py::array::forcecast> array_1(input_layer_1);
        py::buffer_info buffer1 = array_1.request();
        if (buffer1.ndim != 1) {
            char msg[256];
            snprintf(msg, sizeof(msg), "expected input of 1D array for input_layer_1\n");
            throw std::runtime_error(msg);
        }
        size_t num_elements_1 = buffer1.shape[0];
        hnswlib::labeltype* linked_list_1 = static_cast<hnswlib::labeltype*>(buffer1.ptr);

        // load list for layer 2
        py::array_t<hnswlib::labeltype, py::array::c_style | py::array::forcecast> array_2(input_layer_2);
        py::buffer_info buffer2 = array_2.request();
        if (buffer2.ndim != 1) {
            char msg[256];
            snprintf(msg, sizeof(msg), "expected input of 1D array for input_layer_2\n");
            throw std::runtime_error(msg);
        }
        size_t num_elements_2 = buffer2.shape[0];
        hnswlib::labeltype* linked_list_2 = static_cast<hnswlib::labeltype*>(buffer2.ptr);

        // set the graph for each layer
        std::vector<hnswlib::labeltype*> linked_list_vector = {linked_list_1, linked_list_2};
        std::vector<size_t> num_elements_vector = {num_elements_1, num_elements_2};
        appr_alg->setPivotsAndLinks(linked_list_vector, num_elements_vector, M);
    ...
    // MODIFIED: ADDED THIS FOR OUR NEW APPROACH
    py::object knnQuery_hnsw(py::object input, size_t k = 1, int num_threads = -1) {
        ...
        if (num_threads <= 0) num_threads = num_threads_default;
        {
            ...
            if (normalize == false) {
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                    std::priority_queue<std::pair<dist_t, hnswlib::labeltype>> result = appr_alg->search_hnsw((void*)items.data(row), k);
                    if (result.size() != k)
                ...
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                    ...
                    std::priority_queue<std::pair<dist_t, hnswlib::labeltype>> result = appr_alg->search_hnsw((void*)(norm_array.data() + start_idx), k);
                    if (result.size() != k)
                        throw std::runtime_error(
                            "Cannot return the results in a contigious 2D array. Probably ef or M is too small");
                    for (int i = k - 1; i >= 0; i--) {
    ...
    py::object knnQuery_beam(py::object input, size_t k = 1, int num_threads = -1) {
        ...
        if (num_threads <= 0) num_threads = num_threads_default;
        {
            ...
            if (normalize == false) {
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                    std::priority_queue<std::pair<dist_t, hnswlib::labeltype>> result = appr_alg->search_beam((void*)items.data(row), k);
                    if (result.size() != k)
                        throw std::runtime_error("Cannot return the results in a contigious 2D array. Probably ef or M is too small");
                    for (int i = k - 1; i >= 0; i--) {
                        auto& result_tuple = result.top();
                ...
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                    ...
                    std::priority_queue<std::pair<dist_t, hnswlib::labeltype>> result = appr_alg->search_beam((void*)(norm_array.data() + start_idx), k);
                    if (result.size() != k)
                        throw std::runtime_error(
                            "Cannot return the results in a contigious 2D array. Probably ef or M is too small");
                    for (int i = k - 1; i >= 0; i--) {
   ...
   public:
    ...
    BFIndex(const std::string& space_name, const int dim) : space_name(space_name), dim(dim) {
        ...
        if (space_name == "l2") {
            ...
            space = new hnswlib::InnerProductSpace(dim);
            //normalize = true; EDIT: NORMALIZED AHEAD OF TIME
        } else {
            throw std::runtime_error("Space name must be one of l2, ip, or cosine.");
        }
        alg = NULL;
    ...
    void addItems(py::object input, py::object ids_ = py::none()) {
        ...
        if (features != (size_t) dim) throw std::runtime_error("Wrong dimensionality of the vectors");
...
PYBIND11_PLUGIN(hnswlib) {
    ...
    py::class_<Index<float>>(m, "Index")
        ...
        .def("pivot_selection_2L", &Index<float>::pivot_selection_2L, 
                py::arg("radius"), 
                py::arg("num_threads") = 1)
        .def("pivot_selection_3L", &Index<float>::pivot_selection_3L, 
                py::arg("radius1"), 
                py::arg("radius2"),
                py::arg("num_threads") = 1)
        .def("hsp_graph", &Index<float>::HSP_Graph, 
                py::arg("data"), 
                py::arg("k"), 
                py::arg("num_threads") = 1)
        .def("set_pivots_and_links", &Index<float>::set_pivots_and_links, 
                py::arg("graph1"), 
                py::arg("graph2"),
                py::arg("M"))
        .def("select_and_graph", &Index<float>::select_and_graph, 
            py::arg("radius1"), 
            py::arg("radius2"), 
            py::arg("max_neighborhood")=10000, 
            py::arg("max_neighbors") = 10)
        .def("knn_hnsw", &Index<float>::knnQuery_hnsw, 
                py::arg("data"), 
                py::arg("k") = 1,
                py::arg("num_threads") = -1)
        .def("knn_beam", &Index<float>::knnQuery_beam, 
                py::arg("data"), 
                py::arg("k") = 1,
                py::arg("num_threads") = -1);
