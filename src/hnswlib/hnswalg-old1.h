...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
   public:
    ...
    size_t data_size_{0};
    // dist_t* data_pointer_{nullptr};   // MODIFIED: POINTER TO HOLD ALL DATA. OWNED BY PYTHON.
    char* data_pointer_{nullptr};   // MODIFIED: POINTER TO HOLD ALL DATA. OWNED BY PYTHON.
    ...
    // this ONLY gives the data representation
    inline char *getDataVectorByInternalId(tableint internal_id) const {
        // return (char*) (data_pointer_ + internal_id * data_size_);
    ...
    searchBaseLayer(tableint ep_id, const void *data_point, int layer) {
        ...
        if (!isMarkedDeleted(ep_id)) {
            dist_t dist = fstdistfunc_(data_point, getDataVectorByInternalId(ep_id), dist_func_param_);
...
// #endif
                ...
                char *currObj1 = (getDataVectorByInternalId(candidate_id));
    ...
    void getNeighborsByHeuristic2(
        ...
        while (queue_closest.size()) {
            ...
            for (std::pair<dist_t, tableint> second_pair : return_list) {
                dist_t curdist = fstdistfunc_(getDataVectorByInternalId(second_pair.second),
                                              getDataVectorByInternalId(curent_pair.second), dist_func_param_);
        ...
        for (size_t idx = 0; idx < selectedNeighbors.size(); idx++) {
            ...
            if (!is_cur_c_present) {
                if (sz_link_list_other < Mcurmax) {
                    ...
                    dist_t d_max = fstdistfunc_(getDataVectorByInternalId(cur_c), getDataVectorByInternalId(selectedNeighbors[idx]),
                                                dist_func_param_);
                    // Heuristic:
                    ...
                    for (size_t j = 0; j < sz_link_list_other; j++) {
                        candidates.emplace(fstdistfunc_(getDataVectorByInternalId(data[j]),
                                                        getDataVectorByInternalId(selectedNeighbors[idx]), dist_func_param_),
                                           data[j]);
                    }
...
//     }
    ...
    tableint addPoint(const void *data_point, labeltype label, int level) {
        ...
        // memcpy(getDataByInternalId(cur_c), data_point, data_size_); // TO REMOVE: WON'T BE NEEDED
        ...
        if ((signed)currObj != -1) {
            if (curlevel < maxlevelcopy) {
                dist_t curdist = fstdistfunc_(data_point, getDataVectorByInternalId(currObj), dist_func_param_);
                for (int level = maxlevelcopy; level > curlevel; level--) {
                    ...
                    while (changed) {
                        ...
                        for (int i = 0; i < size; i++) {
                            ...
                            dist_t d = fstdistfunc_(data_point, getDataVectorByInternalId(cand), dist_func_param_);
                            if (d < curdist) {
                                curdist = d;
                                currObj = cand;
                                changed = true;
            ...
            for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
                ...
                if (epDeleted) {
                    top_candidates.emplace(
                        fstdistfunc_(data_point, getDataVectorByInternalId(enterpoint_copy), dist_func_param_),
                        enterpoint_copy);
                    if (top_candidates.size() > ef_construction_) top_candidates.pop();
                }
    ...
    std::priority_queue<std::pair<dist_t, labeltype>> searchKnn(const void *query_data, size_t k,
        ...
        dist_t curdist = fstdistfunc_(query_data, getDataVectorByInternalId(enterpoint_node_), dist_func_param_);
        ...
        for (int level = maxlevel_; level > 0; level--) {
            ...
            while (changed) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    dist_t d = fstdistfunc_(query_data, getDataVectorByInternalId(cand), dist_func_param_);
    ...
    void HSP_Test(labeltype queryIndex, std::vector<labeltype> pivots_list, int k, std::vector<labeltype> &neighbors) {
        neighbors.clear();
        char *q_data = getDataVectorByInternalId(queryIndex);
        ...
        for (int it1 = 0; it1 < (int)pivots_list.size(); it1++) {
            ...
            dist_t const d = fstdistfunc_(q_data, getDataVectorByInternalId(index), dist_func_param_);
            if (d < distance_Q1) {
                distance_Q1 = d;
                index1 = index;
            }
        ...
        while (L.size() > 0) {
            ...
            char *index1_data = getDataVectorByInternalId(index1);
            ...
            for (int it1 = 0; it1 < (int)L_copy.size(); it1++) {
                ...
                dist_t const distance_12 = fstdistfunc_(index1_data, getDataVectorByInternalId(index2), dist_func_param_);
    ...
    void topLayerPivotSelection(dist_t radius, int numThreads, std::vector<labeltype> &pivots,
        ...
        for (labeltype p = 0; p < cur_element_count; p++) {
            ...
            pivots.push_back(p);
            vec_covered[p] = true;
            char *pivot_data = getDataVectorByInternalId(p);
...
#pragma omp parallel for schedule(static) num_threads(numThreads)
            for (labeltype x = p + 1; x < cur_element_count; x++) {
                ...
                dist_t distance = fstdistfunc_(pivot_data, getDataVectorByInternalId(x), dist_func_param_);
                if (distance <= radius) {
                    vec_covered[x] = true;
                }
            }
...
#pragma omp parallel for schedule(static) num_threads(numThreads)
        for (labeltype x = 0; x < cur_element_count; x++) {
            char *element_data = getDataVectorByInternalId(x);
            ...
            for (int itp = 0; itp < (int)pivots.size(); itp++) {
                ...
                dist_t distance = fstdistfunc_(element_data, getDataVectorByInternalId(pivot_index), dist_func_param_);
    ...
    void secondLayerPivotSelection(dist_t radius, std::vector<labeltype> const &partition, int numThreads,
        ...
        if (pivots.size() >= 1) {
            ...
            char *pivot1_data = getDataVectorByInternalId(pivot_index1);
            ...
            for (int itx = 0; itx < numElements; itx++) {
                ...
                dist_t distance = fstdistfunc_(pivot1_data, getDataVectorByInternalId(element_index), dist_func_param_);
                if (distance <= radius) {
                    vec_covered[itx] = true;
                }
            }
        ...
        for (int itp = 0; itp < numElements; itp++) {
            ...
            char *pivot_data = getDataVectorByInternalId(pivot_index);
            ...
            for (int itx = itp + 1; itx < numElements; itx++) {
                ...
                labeltype element_index = partition[itx];
                dist_t distance = fstdistfunc_(pivot_data, getDataVectorByInternalId(element_index), dist_func_param_);
                if (distance <= radius) {
                    vec_covered[itx] = true;
                }
            }
...
#pragma omp parallel for schedule(static) num_threads(numThreads)
    ...
    std::priority_queue<std::pair<dist_t, labeltype>> search_hnsw(const void *query_data, size_t k) const {
        ...
        dist_t dist = fstdistfunc_(query_data, getDataVectorByInternalId(currObj), dist_func_param_);
        // dist_t dist = fstdistfunc_(query_data, getDataVectorByInternalId(currObj), dist_func_param_);
...
// #endif
    ...
    std::priority_queue<std::pair<dist_t, labeltype>> search_beam(const void *query_data, size_t k) const {
        ...
        for (size_t i = 0; i < ef; i++) {
            ...
            dist_t d = fstdistfunc_(query_data, getDataVectorByInternalId(random_node), dist_func_param_);
            ...
            while (candidate_set.size() > 0) {
                ...
                if (!(visited_array[current_node_id] == visited_array_tag)) {
                    ...
                    for (size_t j = 1; j <= size; j++) {
                        ...
                        if (!(visited_array[candidate_id] == visited_array_tag)) {
                            visited_array[candidate_id] = visited_array_tag;
                            dist_t dist = fstdistfunc_(query_data, getDataVectorByInternalId(candidate_id), dist_func_param_);
